...
async function verification(method) {
  ...
  try {
    // 1. 포트원 본인인증을 호출하고 사용자가 완료할 때까지 기다립니다.
    ...
    // 2. 사용자가 창을 닫거나 인증에 실패한 경우 에러를 발생시켜 중단합니다.
    if (portoneResponse.code != null) {
      throw new Error(portoneResponse.message);
    }

    // 3. 백엔드에 identityVerificationId를 보내 최종 검증을 요청합니다.
    ...
    // 4. [핵심] 백엔드가 보낸 JSON 응답을 객체로 변환합니다.
    const userInfo = await backendResponse.json();

    // 5. 백엔드로부터 받은 상태가 'success'가 아니면 에러를 발생시켜 중단합니다.
    if (userInfo.status !== 'success') {
      throw new Error(userInfo.message || "서버에서 인증 처리에 실패했습니다.");
    }

    // 6. [성공] 모든 검증이 끝났으므로, 이제 화면을 전환합니다.
    // 단계 표시 변경
    ...
    // 모든 에러(인증 실패, 통신 오류 등)를 여기서 한 번에 처리합니다.
    alert(error.message);
  }
}
