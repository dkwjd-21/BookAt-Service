...
//        .httpBasic(Customizer.withDefaults())
        .authorizeHttpRequests(auth -> auth
        		...
        		.requestMatchers("/", "/user/**", "/auth/**", "/books/**", "/events/**", "/infoPage/**","/cart/**", "/order/orderList", "/order/orderList/**").permitAll()	// 비로그인도 접근 가능
        		.requestMatchers("/api/**", "/queue/**", "/reservation/**", "/myPage/**","/order/**","/order/direct/**").authenticated()	// 로그인 한 사용자만 접근 가능
                .anyRequest().denyAll()
